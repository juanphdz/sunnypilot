...
class SelfdriveD(CruiseHelper):
  def __init__(self, CP=None, CP_SP=None):
    ...
    if SIMULATION:
      ignore += ['driverCameraState', 'managerState', 'wideRoadCameraState', 'audioFeedback']
    if REPLAY:
  ...
  def update_events(self, CS):
    ...
    if self.sm.recv_frame['managerState'] and (not_running - self.ignored_processes) and not SIMULATION:
    ...
    if not self.sm.all_checks() and no_system_errors and not SIMULATION:
    ...
    if any((self.sm.frame - self.sm.recv_frame[s])*DT_CTRL > 10. for s in self.sensor_packets) and not SIMULATION:
